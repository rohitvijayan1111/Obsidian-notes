...
-- TABLE: users
--
-- Stores the profile of every person in the system.
-- Auth details live in user_credentials (separate table).
-- Role assignments live in user_roles (separate table).
--
-- INSERT: When Institute Admin or Department Admin creates
--         a new user account. There is no self-signup.
-- UPDATE: When profile details change (name, department, email).
-- SOFT DELETE: deleted_at set when account is permanently deactivated.
-- ============================================================
...
CREATE TABLE users (
    ...
    phone         TEXT,
    preferences   JSONB       NOT NULL DEFAULT '{}',  -- UI language, notification prefs
    created_by    UUID        REFERENCES users (id),  -- the admin who created this account
...
CREATE INDEX idx_users_institute   ON users (institute_id)  WHERE deleted_at IS NULL;
